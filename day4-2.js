...
var test = ["abcde fghij",
"abcde xyz ecdab"];


//solve(input);
//solveSmart(input);
solveSuperSmart(test);
solveSuperSmart(input);
    ...
    input.forEach(element => {
        ...
        for (var index = 0; index < words.length; index++) {
            ...
            for (var letterIndex = 0; letterIndex < word.length; letterIndex++) {
                letters[letterIndex] = word.substr(letterIndex, 1);
            }
            var sortedWord = letters.sort().join('');
            if (dupes[sortedWord]) {
                valid = false;
                break;
            } else {
                dupes[sortedWord] = true;
            }
        }
        valid ? valids++ : invalids++;
    });
...
function solveSuperSmart (input) {
    var dupes = {};

    var countMe = input.reduce((prevVal, password) => {
        var dupes = {};

        var isPasswordValid = password.split(" ")
            .map(word => [...word].sort())
            .reduce((isValid, sortedWord) => {
                if (dupes[sortedWord]) {
                    return false;
                } else {
                    dupes[sortedWord] = true;
    ...
    console.log("valid: " + countMe.valids);
    console.log("invalid: " + countMe.invalids);
}
