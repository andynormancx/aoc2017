...
//solve(testInput, 'pbga');
solve(input, 'hndzyx');

function solve(input, firstDiscName) {
    let discs = {};

    let lines = input.map(el => el.split(" -> "));

    lines.forEach(line => {
        let parts = line[0].split(" ");
        let weight = parseInt(parts[1].replace("(", "").replace(")", ""))
        let disc = {
            name: parts[0],
            weight: weight,
            totalWeight: 0,
            childNames: line.length == 2 ? line[1].split(", ") : [],
            childDiscs: [],
            parent: undefined
        };
        addDisc(disc, discs);
    ...
    Object.keys(discs).forEach(childDiscName => {
        let disc = discs[childDiscName];

        disc.childNames.forEach(el => {
            disc.childDiscs.push(discs[el]);
        });

        Object.keys(discs).forEach(possibleParentDiscName => {
            let possibleParentDisc = discs[possibleParentDiscName];
            possibleParentDisc.childNames.forEach(el => {
                if (el == childDiscName) {
                    discs[childDiscName].parent = possibleParentDisc;
                }
            });
        })
    });
    ...
    Object.keys(discs).forEach(childDiscName => {
        if (discs[childDiscName].parent == undefined) {
            topDisc = discs[childDiscName];
            console.log("no parent: " + childDiscName);
        }
    });

    sumChildWeights(topDisc);
    function sumChildWeights(disc) {
        let sumWeight = 0;

        if (disc.childDiscs.length == 0) {
            disc.totalWeight = disc.weight;
            return disc.weight;
        } else {
            let weight = -1;
            let dodgy = false;
            disc.childDiscs.forEach(child => {
                let childWeight = sumChildWeights(child);
            ...
            disc.totalWeight = disc.weight + sumWeight;
            return disc.totalWeight;    
        }
    }
    

    function addDisc (disc, discs) {
        if (discs[disc.name] == undefined) {
            discs[disc.name] = disc;
        } else {
            discs[disc.name].weight = disc.weight;
            discs[disc.name].childNames = disc.childNames.split(", ");
        }
    }
}