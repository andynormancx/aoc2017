...
function solve(input) {
    ...
    let maxDuring = 0;
    lines.forEach(line => {
        let register = getReg(line.register);
        let compareToReg = getReg(line.condition.register);

        if (eval(compareToReg.value + line.condition.op + line.condition.value)) {
            if (line.op == 'inc') {
                register.value += line.change;
            } else {
                register.value -= line.change;                
            }
        }

        if (register.value > maxDuring) {
            maxDuring = register.value;
        }
    });

    console.log("max during: " + maxDuring);
    ...
    */
    ...
    function getReg(name) {
        if (!registers.has(name)) {
            registers.set(name, { name: name, value: 0});
        }
