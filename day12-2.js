...
function solve(input, programToCheck) {
    let lines = input.map(line => {
        let parts = line.split(' <-> ');
        let programs = parts[1].split(', ');  
        return {            
            program: parts[0],
            connectionNames: programs,
            connections: new Map()
        };
    });

    let programsDict = new Map(); 

    lines.forEach(line => {
        programsDict.set(line.program, line);
    });

    programsDict.forEach(program => {
        program.connectionNames.forEach(programName => {
            program.connections.set(programName, programsDict.get(programName));
        });
    });
    ...
    programsDict.forEach(program => {
        let childConnectionNames = new Set();
        search(program, childConnectionNames);
        groups.add(Array.from(childConnectionNames).sort().join(","));
    });
    ...
    function search(program, childConnectionNames) {
        if (childConnectionNames.has(program.program)) return;
        childConnectionNames.add(program.program);
        program.connections.forEach(connection => {
            search(connection, childConnectionNames);
        });
    }
}
...
console.log(solve(input).connections.size);